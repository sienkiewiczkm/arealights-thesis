...
Próbki Monte-Carlo można wybierać w sposób całkowicie losowy, jednak taka
metoda nie daje nam żadnej gwarancji, że wybrane próbki będą pokrywały
dziedzinę w sposób równomierny. Z pomocą przychodzą nam ciągi
\textit{subrandom, quasi-random}\todo{Przetłumaczyć.}, które pokrywają zadaną
dziedzinę w sposób całkowity i równomierny. Załóżmy, że nasza dziedzina jest
kostką $n$ wymiarową: $[0,1]^{n}$.

Pierwszą metodą jednorodną jaka się narzuca jest zbudowanie równomiernej siatki
posiadającej $m$ punktów na boku kostki. Problemem tej metody jest jej
ograniczoność, zdefiniowaliśmy dokładnie $m^{n}$ punktów, które musimy
wykorzystać, aby zbiór mógł być uważany za jednorodny, nie możemy punktów
usuwać ani dodawać.

Ciągi o niskiej rozbieżności są rozwiązaniem tego problemu, są to ciągi
deterministyczne, które wydają się losowe oraz pokrywają równomiernie całą
dziedzinę niezależnie od ilości punktów.

Najbardziej znanymi ciągami tego typu są: ciąg Haltona, zbiór Hammersleya,
ciąg Sobola. W tej pracy zostaną omówione i wykorzystane dwa pierwsze.

\subsection{Ciąg van der Corput'a}

Ciąg van der Corput'a jest jednowymiarowym ciągiem o niskiej rozbieżności
zdefiniowanym na zbiorze $[0,1]$ zbudowanym poprzez odwrócenie bazy
reprezentacji w danym systemie o podstawie $b$. Każda liczba całkowita $i$ może
zostać przedstawiona w pewnej zadanej bazie w sposób następujący:
...
Operacja bitowa tego typu pozwoli na uzyskanie poprawnego wyniku.
...
Do wygenerowania punktów na sferze możemy wykorzystać dowolną z przedstawionych
metod do wygenerowania $v = (\phi, t) \in [0,1]^2$. \todo{Dokończyć.}
